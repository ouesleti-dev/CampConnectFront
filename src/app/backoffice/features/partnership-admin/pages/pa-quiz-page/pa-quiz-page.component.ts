...
export class PaQuizPageComponent implements OnInit, OnDestroy {
  ...
  qCols = [{ key: 'id', label: 'ID' }, { key: 'titre', label: 'Titre' }, { key: 'partenaireLabel', label: 'Partenaire' }];
  quCols = [{ key: 'id', label: 'ID' }, { key: 'quizLabel', label: 'Quiz' }, { key: 'texte', label: 'Question' }, { key: 'type', label: 'Type' }, { key: 'points', label: 'Pts' }];
  rCols = [{ key: 'id', label: 'ID' }, { key: 'questionLabel', label: 'Question' }, { key: 'partenaireLabel', label: 'Partenaire' }, { key: 'valeur', label: 'Réponse' }, { key: 'score', label: 'Score' }];
  ...
  private rebuild(): void {
    ...
    this.quizRows = s.quizzes.map((q) => ({ ...q, partenaireLabel: s.users.find((u) => u.id === q.partenaireId)?.lastName || q.partenaireId }));
    this.questionRows = s.questions.map((q) => ({ ...q, quizLabel: s.quizzes.find((z) => z.id === q.quizId)?.titre || q.quizId }));
    ...
    this.chartByPartner = { type: 'bar', data: { labels: plabels, datasets: [{ label: 'Score cumulé', data: [...byPartner.values()], backgroundColor: '#86b7fe' }] }, options: { plugins: { legend: { display: false } }, maintainAspectRatio: false } };
    const byQuiz = new Map<number, number>();
    for (const r of s.reponses) { const q = s.questions.find((x) => x.id === r.questionId); if (q) byQuiz.set(q.quizId, (byQuiz.get(q.quizId) || 0) + r.score); }
    const qlabels = [...byQuiz.keys()].map((id) => s.quizzes.find((z) => z.id === id)?.titre || String(id));
    this.chartByQuiz = { type: 'doughnut', data: { labels: qlabels, datasets: [{ data: [...byQuiz.values()], backgroundColor: ['#0d6efd', '#6ea8fe', '#cfe2ff', '#ffc107'] }] }, options: { plugins: { legend: { position: 'bottom' } }, maintainAspectRatio: false } };
  }
  ...
  saveQuiz(): void {
    if (this.quizForm.invalid) { this.quizForm.markAllAsTouched(); this.toast.warning('Veuillez corriger les erreurs du formulaire.'); return; }
    const v = this.quizForm.getRawValue();
    if (this.editingQuiz != null) { this.store.updateQuiz(this.editingQuiz, { titre: v.titre!, partenaireId: Number(v.partenaireId) }); this.toast.success('Quiz mis à jour'); }
    else { this.store.addQuiz({ titre: v.titre!, partenaireId: Number(v.partenaireId) }); this.toast.success('Quiz créé'); }
    this.showQuiz = false;
  }

  delQuiz(row: Record<string, unknown>): void {
    if (confirm('Supprimer ce quiz et ses questions ?')) { this.store.deleteQuiz(row['id'] as number); this.toast.success('Quiz supprimé'); }
  }
  ...
  saveQuestion(): void {
    if (this.questionForm.invalid) { this.questionForm.markAllAsTouched(); this.toast.warning('Veuillez corriger les erreurs du formulaire.'); return; }
    ...
    if (this.editingQuestion != null) { this.store.updateQuestion(this.editingQuestion, payload); this.toast.success('Question mise à jour'); }
    else { this.store.addQuestion(payload); this.toast.success('Question ajoutée'); }
    this.showQuestion = false;
  }

  delQuestion(row: Record<string, unknown>): void {
    if (confirm('Supprimer cette question ?')) { this.store.deleteQuestion(row['id'] as number); this.toast.success('Question supprimée'); }
  }
  ...
  saveReponse(): void {
    if (this.reponseForm.invalid) { this.reponseForm.markAllAsTouched(); this.toast.warning('Veuillez corriger les erreurs du formulaire.'); return; }
    ...
    this.toast.success('Réponse enregistrée (score calculé)');
    this.showReponse = false;
  }

  delReponse(row: Record<string, unknown>): void {
    if (confirm('Supprimer cette réponse ?')) { this.store.deleteReponse(row['id'] as number); this.toast.success('Réponse supprimée'); }
  }
}
