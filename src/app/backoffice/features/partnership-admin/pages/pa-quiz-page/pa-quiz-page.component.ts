...
export class PaQuizPageComponent implements OnInit, OnDestroy {
  ...
  quCols = [
    { key: 'id', label: 'ID' },
    { key: 'quizLabel', label: 'Quiz' },
    { key: 'texte', label: 'Question' },
    { key: 'type', label: 'Type' },
    { key: 'points', label: 'Pts' },
  ];
  rCols = [
    { key: 'id', label: 'ID' },
    { key: 'questionLabel', label: 'Question' },
    { key: 'partenaireLabel', label: 'Partenaire' },
    { key: 'valeur', label: 'Réponse' },
  ...
  private syncQuestionOptionsValidators(): void {
    ...
    oj?.clearValidators();
    if (t === 'QCM') {
      oj?.setValidators([Validators.required, optionalJsonArrayValidator]);
    } else {
      oj?.setValidators([optionalJsonArrayValidator]);
    }
    oj?.updateValueAndValidity({ emitEvent: false });
  }

  qzInvalid(name: string): boolean {
    return showFieldError(this.quizForm.get(name));
  }
  qzMsg(name: string): string {
    const c = this.quizForm.get(name);
    return c?.errors ? validationMessage(c.errors) : '';
  }
  qnInvalid(name: string): boolean {
    return showFieldError(this.questionForm.get(name));
  }
  qnMsg(name: string): string {
    const c = this.questionForm.get(name);
    return c?.errors ? validationMessage(c.errors) : '';
  }
  rpInvalid(name: string): boolean {
    return showFieldError(this.reponseForm.get(name));
  }
  rpMsg(name: string): string {
    const c = this.reponseForm.get(name);
    return c?.errors ? validationMessage(c.errors) : '';
  }
  ...
  private rebuild(): void {
    ...
    for (const r of s.reponses) {
      byPartner.set(r.partenaireId, (byPartner.get(r.partenaireId) || 0) + r.score);
    }
    ...
    this.chartByPartner = {
      ...
      data: {
        labels: plabels,
        datasets: [{ label: 'Score cumulé', data: [...byPartner.values()], backgroundColor: '#86b7fe' }],
      },
    ...
    this.chartByQuiz = {
      ...
      data: {
        labels: qlabels,
        datasets: [{ data: [...byQuiz.values()], backgroundColor: ['#0d6efd', '#6ea8fe', '#cfe2ff', '#ffc107'] }],
      },
  ...
  openQuiz(create: boolean, row?: Record<string, unknown>): void {
    ...
    if (create) {
      this.quizForm.reset({ titre: '', partenaireId: null });
    } else {
      const q = this.store.snapshot().quizzes.find((x) => x.id === row!['id']);
      this.quizForm.patchValue({ titre: q!.titre, partenaireId: q!.partenaireId });
    }
    this.showQuiz = true;
  ...
  saveQuiz(): void {
    ...
    if (this.editingQuiz != null) {
      this.store.updateQuiz(this.editingQuiz, { titre: v.titre!, partenaireId: Number(v.partenaireId) });
      this.toast.success('Quiz mis à jour');
    } else {
      this.store.addQuiz({ titre: v.titre!, partenaireId: Number(v.partenaireId) });
  ...
  delQuiz(row: Record<string, unknown>): void {
    if (confirm('Supprimer ce quiz et ses questions ?')) {
      this.store.deleteQuiz(row['id'] as number);
  ...
  openQuestion(create: boolean, row?: Record<string, unknown>): void {
    ...
    if (create) {
      this.questionForm.reset({ quizId: null, texte: '', type: 'OUI_NON', points: 10, optionsJson: '', bonneReponse: '' });
    } else {
  ...
  saveQuestion(): void {
    ...
    const payload = {
      quizId: Number(v.quizId),
      texte: v.texte!,
      type: v.type as QuestionType,
      points: Number(v.points),
      optionsJson: v.optionsJson?.trim() || undefined,
      bonneReponse: v.bonneReponse?.trim() || undefined,
    };
    if (this.editingQuestion != null) {
      this.store.updateQuestion(this.editingQuestion, payload);
  ...
  delQuestion(row: Record<string, unknown>): void {
    if (confirm('Supprimer cette question ?')) {
      this.store.deleteQuestion(row['id'] as number);
  ...
  delReponse(row: Record<string, unknown>): void {
    if (confirm('Supprimer cette réponse ?')) {
      this.store.deleteReponse(row['id'] as number);
