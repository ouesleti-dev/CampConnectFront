...
export class PaRencontresPageComponent implements OnInit, OnDestroy {
  ...
  private rebuild(): void {
    ...
    if (this.filterEntretienId != null) {
      list = list.filter((r) => r.entretienId === this.filterEntretienId);
    }
    this.rows = list
      .slice()
  ...
  openCreate(): void {
    this.editingId = null;
    this.form.reset({
      entretienId: this.filterEntretienId,
  ...
  save(): void {
    ...
    const payload = {
      entretienId: Number(v.entretienId),
      date: v.date!,
      mode: v.mode as EntretienMode,
      compteRendu: v.compteRendu!,
    };
    if (this.editingId != null) {
      this.store.updateRencontre(this.editingId, payload);
      this.toast.success('Rencontre mise à jour');
    } else {
      this.store.addRencontre(payload);
      this.toast.success('Rencontre ajoutée');
    }
  ...
  remove(row: Record<string, unknown>): void {
    if (confirm('Supprimer cette rencontre ?')) {
      this.store.deleteRencontre(row['id'] as number);
      this.toast.success('Rencontre supprimée');
    }
  }

  rencontresForEntretien(eid: number): void {
    this.filterEntretienId = eid;
    this.applyFilter();
  }
}
